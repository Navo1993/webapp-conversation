...
const Main: FC<IMainProps> = () => {
  ...
  */
  ...
  const handleSend = async (message: string, files?: VisionFile[]) => {
    ...
    // ================================================================
    // 🎙️ [新增] STEP 1: 检测是否为语音消息
    //
    // Chat 子组件上传音频后，会传入一个 type === 'audio' 的 VisionFile。
    // 我们在这里识别它，并将其转换为 Dify Chatflow 所需的 inputs 字段，
    // 而不是作为普通文件传递（Dify 的 files 参数不支持 audio 类型）。
    // ================================================================
    const audioFile = files?.find(f => (f.type as string) === 'audio')
    const isVoiceMessage = !!audioFile

    // voice inputs：只在语音消息时追加，文本消息时为空对象（零影响）
    const voiceInputs: Record<string, string> = isVoiceMessage
      ? {
          input_type: 'voice',                          // Chatflow 开始节点识别字段
          voice_file_id: audioFile!.upload_file_id,     // 已上传文件的 ID
        }
      : {}

    // 过滤掉 audio 类型，不传入 Dify files 参数
    // （Dify files 只接受 image/document 等标准类型）
    ...
    const data: Record<string, any> = {
      // 🎙️ [改动] 将 voiceInputs 合并进 inputs（文本消息时 voiceInputs 为 {}，无任何影响）
      inputs: { ...toServerInputs, ...voiceInputs },
      query: message,                                  // 语音消息时 message 为 ''
      conversation_id: isNewConversation ? null : currConversationId,
    }

    // 🎙️ [改动] 使用过滤后的 nonAudioFiles（排除了 audio 类型）
    ...
    const questionItem = {
      id: questionId,
      // 🎙️ 语音消息时 content 为 ''，显示为空气泡（可按需改为"🎙️ 语音消息"）
      content: isVoiceMessage ? '🎙️ 语音消息' : message,
      isAnswer: false,
      // audio 类型不作为 image 展示，过滤掉
      message_files: (nonAudioFiles).filter((f: any) => f.type === 'image'),
    }
    ...
    sendChatMessage(data, {
      ...
      onWorkflowStarted: ({ workflow_run_id, task_id }) => {
        ...
        setChatList(produce(getChatList(), (draft) => {
          ...
          draft[currentIndex] = { ...draft[currentIndex], ...responseItem }
        }))
      },
    ...
    <div className="bg-gray-100">
      ...
      <div className="flex rounded-t-2xl bg-white overflow-hidden">
        ...
        <div className="flex-grow flex flex-col h-[calc(100vh_-_3rem)] overflow-y-auto">
          ...
          {hasSetInputs && (
            <div
              className="relative grow pc:w-[794px] max-w-full mobile:w-full pb-[180px] mx-auto mb-3.5"
              ref={chatListDomRef}
            >
              <Chat
                chatList={chatList}
