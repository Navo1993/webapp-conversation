...
/* ================================================================
   🎙️ 语音输入 Hook（火山流式识别版）
   录音(webm) → 转换(wav) → WebSocket 流式发送 → 收到文字 → onSend
   ================================================================ */
function useVoiceInput(
  ...
  /** 通过 WebSocket 中转发送 wav 给火山识别，返回转录文字 */
  const transcribeViaWS = useCallback((wavBlob: Blob): Promise<string> => {
    return new Promise((resolve, reject) => {
      const wsUrl = process.env.NEXT_PUBLIC_ASR_WS_URL || 'ws://159.75.185.246:8765'
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (data.text)
            finalText = data.text // 持续更新，取最终结果

          if (data.is_final && finalText) {
            clearTimeout(timeout)
            ws.close()
            resolve(finalText)
          }
      ...
      ws.onerror = () => {
        clearTimeout(timeout)
        reject(new Error('WebSocket 连接失败，请检查服务器'))
      }

      ws.onclose = () => {
        clearTimeout(timeout)
        // 如果关闭时有文字但没收到 is_final，也返回
        if (finalText)
          resolve(finalText)
        else
          reject(new Error('识别结果为空，请重新录音'))
      }
    })
  }, [])
  ...
  const stopAndSend = useCallback(() => {
    ...
    recorder.onstop = async () => {
      ...
      try {
        ...
        const text = await transcribeViaWS(wavBlob)
        onSend(text, [])
      }
      catch (err: any) {
    ...
    recorder.stop()
  }, [transcribeViaWS, onSend, logError])
