...
const content = {
  zh_cn: {
    ...
    common: { start: '立即体验', langName: '简体中文', more: '了解更多' },
    hero: { 
      title1: '连接安全', 
      title2: '预见未来智能', 
    ...
    faq: {
      ...
      subtitle: 'FAQs',
      items: [
        { q: 'Smart Guard 如何确保数据的实时性？', a: '我们通过自主研发的 IoT 网关直接对接默纳克控制系统，实现毫秒级的数据采集与云端同步。' },
        { q: '系统是否支持远程控制电梯？', a: '出于安全合规考虑，目前系统仅提供监测、预警与诊断建议，不直接参与电梯的运行控制决策。' }
      ]
    },
    footer: { 
  ...
  zh_tw: {
    ...
    common: { start: '立即體驗', langName: '繁體中文', more: '瞭解更多' },
    hero: { 
      title1: '連接安全', 
      title2: '預見未來智能', 
    ...
    faq: {
      ...
      subtitle: 'FAQs',
      items: [
        { q: 'Smart Guard 如何確保數據的實時性？', a: '我們通過自主研發的 IoT 網關直接對接默納克控制系統，實現毫秒級的數據採集與雲端同步。' },
        { q: '系統是否支持遠程控制電梯？', a: '出於安全合規考慮，目前系統僅提供監測、預警與診斷建議，不直接參與電梯的運行控制決策。' }
      ]
    },
    footer: { 
  ...
  en: {
    nav: [
      ...
      { id: 'tech', label: 'Technology', columns: [
        { title: 'AI Engine', links: ['Dify Training', 'Fault Prediction', 'RAG Base'] },
        { title: 'IoT', links: ['Monarch Protocol', 'Sensor Fusion', 'Digital Retrofit'] }
      ]},
      { id: 'news', label: 'Updates', columns: [
        { title: 'Latest', links: ['Releases', 'Industry News', 'Review'] }
      ]}
    ],
    common: { start: 'Get Started', langName: 'English', more: 'Learn More' },
    hero: { 
    ...
    faq: {
      ...
      items: [
        { q: 'How is real-time data ensured?', a: 'Via our IoT gateway connected to Monarch systems with millisecond latency.' },
        { q: 'Does it support remote control?', a: 'For safety compliance, we currently only provide monitoring and alerts.' }
      ]
    },
    footer: { 
  ...
  return (
    <div className="min-h-screen bg-white text-[#1d1d1f] font-sans antialiased selection:bg-blue-100 selection:text-blue-700">
      ...
      >
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="relative">
              <button 
                ...
                className="flex items-center gap-2 text-[13px] font-bold text-gray-500 hover:text-[#0052D9] transition-colors"
              >
                <Globe size={16} />
                {t.common.langName}
              </button>
                  ...
                  >
                    {(['zh_cn', 'zh_tw', 'en'] as LangType[]).map((l) => (
                      <div 
                        ...
                        className={`px-4 py-2 text-[13px] font-medium cursor-pointer hover:bg-blue-50 transition-colors ${lang === l ? 'text-[#0052D9]' : 'text-gray-600'}`}
                      >
        ...
        {/* 下拉菜单逻辑保持不变，确保 handleLinkClick 使用正确 */}
        <AnimatePresence>
          {activeMenu && (
            <motion.div
              ...
              <div className="max-w-[1400px] mx-auto px-8 lg:px-12 py-12 grid grid-cols-4 gap-12">
                {t.nav.find(n => n.id === activeMenu)?.columns.map((col, idx) => (
                  <div key={idx} className="space-y-6">
                    <h4 className="text-[12px] font-black text-gray-400 tracking-widest uppercase">{col.title}</h4>
      ...
      <section className="py-40 px-12 max-w-[1400px] mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {t.features.list.map((f, i) => (
            <div key={i} className="group p-12 bg-[#FBFBFC] rounded-[40px] hover:bg-white hover:shadow-xl transition-all duration-500">
            ...
            <div className="lg:col-span-8 grid grid-cols-2 sm:grid-cols-3 gap-12">
              <div className="space-y-5">
                ...
                <ul className="space-y-3 text-gray-400 font-bold text-sm">
                  <li className="hover:text-white cursor-pointer flex items-center gap-2"><Lock size={14}/> {t.footer.links[0]}</li>
                  <li className="hover:text-white cursor-pointer flex items-center gap-2"><FileText size={14}/> {t.footer.links[1]}</li>
                </ul>
              </div>
            </div>
          </div>
